...
def get_region(valid_regions) -> str:
    """
    ...
    """
    while True:
        region = input(f"Enter region {list(valid_regions.keys())}: ").lower()
        if region in valid_regions.keys():
            return valid_regions[region]
        else:
            print(f'Enter a valid region. The suported regions are {list(valid_regions.keys())}')
